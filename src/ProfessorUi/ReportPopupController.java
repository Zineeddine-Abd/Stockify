...
public class ReportPopupController implements Initializable{
	...
	public void validateReportInfo(ActionEvent event) {
		...
		((ProfessorController)Helper.currentProfessorLoader.getController()).getAllAssetsViewController().updateAsset(oldAsset, new_asset);
		disposeWindow(event);
		((ProfessorController)Helper.currentProfessorLoader.getController()).getDashboardViewController().refreshList();
	}
