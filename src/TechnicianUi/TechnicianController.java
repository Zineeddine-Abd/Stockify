...
public class TechnicianController implements Initializable{
	...
	public void triggerDashBoardPane() {
		if(!recentViews.peek().equals(DASHBOARD_VIEW)) {
			...
			((TechnicianController)Helper.currentTechnicianLoader.getController()).getAllAssetsViewController().searchTextField.setText("");
			updateNumberOfItems();
			selectView(DASHBOARD_VIEW);
			closeSideBar();
	...
	public void popupAccountInfo(ActionEvent event) {
		Parent root;
		try {
			currentPopupAccountInfo = new FXMLLoader(getClass().getResource(TechnicianController.fxmlpopupAccountInfo));
			root = TechnicianController.currentPopupAccountInfo.load();
